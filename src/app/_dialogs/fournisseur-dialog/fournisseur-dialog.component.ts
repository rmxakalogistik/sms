...
import { FournisseurData, FournisseurService } from '../../_services/fournisseur.service';
...
export class FournisseurDialogComponent implements OnInit, AfterContentInit {
  ...
  constructor(
    ...
    @Optional() @Inject(MAT_DIALOG_DATA) public data: FournisseurData,
  ...
  ngOnInit(): void {
    ...
    if (this.action != 'Création') {
      ...
      this.fournisseurProduits = this.local_data.fournisseurProduits || [];
    }
  ...
  canMoveNext(): boolean {
    ...
    return this.formData.valid && this.fournisseurProduits && this.fournisseurProduits.length > 0 && this.tempIndexProduit<0;
  }
  ...
  cancelProduit() {
    this.formDataSubProduit.controls['zone'].reset();
  ...
  editProduit(_data1, i) {
    this.formDataSubProduit.get('zone').setValue(_data1.zone);
  ...
  deleteProduit(i) {
    this.fournisseurProduits.splice(i, 1);
  }

  addProduit(_data1) {

    let _cp = this.fournisseurProduits[this.tempIndexProduit] || {};

    _cp.zone = _data1.zone;
    _cp.zoneId = _data1.zone.id;
    ...
    if (this.tempIndexProduit >= 0) {
      ...
      _cp.fournisseurId = this.local_data.id;

      let _pass = this.fournisseurProduits.filter((x) => {
        return x.zoneId == _cp.zoneId && x.produitId == _cp.produitId;
      })[0];
      if(_pass != null && _pass != undefined){
        _pass.prix = _cp.prix;
      }else{
        this.fournisseurProduits.push(_cp);
      }
    }
  ...
  onSubmit(_data1) {
    ...
    const cp = {
      ...
      fournisseurProduits: this.fournisseurProduits.length == 0 ? [] : this.fournisseurProduits.map((value) => {
        return {
          id: value.id,
          prix: value.prix,
          zoneId: value.zone.id,
          produitId: value.produit.id,
          fournisseurId: this.local_data.id,
        };
      })
    };
    ...
    setTimeout(() => {
      ...
      if (this.action == 'Création') {
        this.fournisseurService.add(cp).pipe(first()).subscribe(data => {
        ...
        this.fournisseurService.update(cp.id, cp).pipe(first()).subscribe(data => {
        ...
        this.fournisseurService.delete(cp.id).pipe(first()).subscribe(data => {
  ...
  async selectionDialog(action, obj: any = {}) {
    ...
    if (action == 'produit') {
      ...
      if(this.fournisseurProduits != undefined && this.fournisseurProduits != null){
        for(let i = 0; i < this.fournisseurProduits.length;i++){
          obj.notLoadData += this.fournisseurProduits[i].id + '';
        }
      }
    ...
    dialogRef.afterClosed().subscribe(async result => {
      ...
      if (!isNullOrUndefined(result) && !isNullOrUndefined(result.data)) {
        if (result.event == "Choix d'un produit") {
          ...
          console.log(result.data[0]);
          this.formDataSubProduit.get('zone').setValue(result.data[0])
        }
