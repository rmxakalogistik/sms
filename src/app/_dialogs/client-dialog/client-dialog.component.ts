...
import { ClientData, ClientService } from '../../_services/client.service';
...
export class ClientDialogComponent implements OnInit, AfterContentInit {
  ...
  ngOnInit(): void {
    ...
    if (this.action != 'Création') {
      ...
      this.clientProduits = this.local_data.clientProduits || [];
    }
  ...
  canMoveNext(): boolean {
    ...
    return this.formData.valid && this.clientProduits && this.clientProduits.length > 0 && this.tempIndexProduit<0;
  }
  ...
  deleteProduit(i) {
    this.clientProduits.splice(i, 1);
  }

  addProduit(_data1) {

    let _cp = this.clientProduits[this.tempIndexProduit] || {};
    ...
    if (this.tempIndexProduit >= 0) {
      ...
      _cp.clientId = this.local_data.id;
      let _pass = this.clientProduits.filter((x) => {
        return x.zoneId == _cp.zoneId && x.produitId == _cp.produitId;
      })[0];
      if(_pass != null && _pass != undefined){
        _pass.prix = _cp.prix;
      }else{
        this.clientProduits.push(_cp);
      }
    }
  ...
  onSubmit(_data1) {
    ...
    const cp = {
      ...
      clientProduits: this.clientProduits.length == 0 ? [] : this.clientProduits.map((value) => {
        return {
          id: value.id,
          prix: value.prix,
          zoneId: value.zone.id,
          produitId: value.produit.id,
          clientId: this.local_data.id,
        };
      })
    };
    ...
    setTimeout(() => {
      ...
      if (this.action == 'Création') {
        this.clientService.add(cp).pipe(first()).subscribe(data => {
        ...
        this.clientService.update(cp.id, cp).pipe(first()).subscribe(data => {
        ...
        this.clientService.delete(cp.id).pipe(first()).subscribe(data => {
  ...
  async selectionDialog(action, obj: any = {}) {
    ...
    if (action == 'produit') {
      ...
      if(this.clientProduits != undefined && this.clientProduits != null){
        for(let i = 0; i < this.clientProduits.length;i++){
          obj.notLoadData += this.clientProduits[i].id + '';
        }
      }
