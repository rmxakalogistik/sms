...
@Component({
  selector: 'app-prelevement-dialog',
  templateUrl: './prelevement-dialog.component.html',
  styleUrls: ['./prelevement-dialog.component.scss']
})
export class PrelevementDialogComponent implements OnInit, AfterContentInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<PrelevementDialogComponent>,
  ...
  ngOnInit(): void {
    ...
    this.prelevementProduits = this.local_data.prelevementProduits || [];
    if(this.local_data && 
  ...
  canMoveNext(): boolean {
    ...
    //return this.formData.valid && this.prelevementProduits && this.prelevementProduits.length > 0 && this.tempIndexProduit < 0;
    return this.prelevementProduits && this.prelevementProduits.length > 0 && this.tempIndexProduit < 0 && !this.formData.valid;
  }
  ...
  deleteProduit(i) {
    //this.prelevementProduits.splice(i, 1);
    this.prelevementProduits[i].deleted = true;
  }

  addProduit(_data1) {

    let _cp = this.prelevementProduits[this.tempIndexProduit] || {};
    ...
    if (this.tempIndexProduit >= 0) {
      ...
      let _pass = this.prelevementProduits.filter((x) => {
        return (x.id == _cp.id);
      })[0];
      if (_pass != null && _pass != undefined) {
        ...
        _cp.id = myLocalId;
        this.prelevementProduits.push(_cp);
      }
    }
    this.cancelProduit();
  }
  ...
  onSubmit() {
    ...
    this.prelevementProduits = this.prelevementProduits.filter((value, key) => {
      return !value.deleted;
    });
    let cp = this.prelevementProduits.length == 0 ? [] : this.prelevementProduits.map((value) => {
    ...
    this.local_data.prelevementProduits = cp;
