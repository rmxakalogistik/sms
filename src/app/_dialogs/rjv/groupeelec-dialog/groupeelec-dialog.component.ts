...
@Component({
  selector: 'app-groupeelec-dialog',
  templateUrl: './groupeelec-dialog.component.html',
  styleUrls: ['./groupeelec-dialog.component.scss']
})
export class GroupeelecDialogComponent implements OnInit, AfterContentInit {
  ...
  // any
  local_data: any;
  groupeelecProduits: any = [];
  zoneProduits: any = [];
  selectedStationProduit: any;
  dialogData: any = null;
  ...
  constructor(
    public dialogRef: MatDialogRef<GroupeelecDialogComponent>,
  ...
  ngOnInit(): void {
    ...
    this.groupeelecProduits = this.local_data.groupeelecProduits || [];
    if(this.local_data && 
      this.local_data.station && 
      this.local_data.station.stationProduits && 
      this.local_data.station.stationProduits.length>0){
        this.selectedStationProduit = this.local_data.station.stationProduits.filter((x)=>{
          return x.produit.nom == 'GASOIL' || x.produit.code == 'GO';
        })[0];
        this.formData.get('stationProduit').setValue(this.selectedStationProduit);
    }
  ...
  canMoveNext(): boolean {
    ...
    //return this.formData.valid && this.groupeelecProduits && this.groupeelecProduits.length > 0 && this.tempIndexProduit < 0;
    return this.groupeelecProduits && this.groupeelecProduits.length > 0 && this.tempIndexProduit < 0 && !this.formData.valid;
  }
  ...
  editProduit(_data1, i) {
    ...
    //this.formData.get('stationProduit').setValue(_data1.stationProduit);
  ...
  deleteProduit(i) {
    //this.groupeelecProduits.splice(i, 1);
    this.groupeelecProduits[i].deleted = true;
  }

  addProduit(_data1) {

    let _cp = this.groupeelecProduits[this.tempIndexProduit] || {};
    ...
    if (this.tempIndexProduit >= 0) {
      ...
      let _pass = this.groupeelecProduits.filter((x) => {
        return (x.id == _cp.id);
      })[0];
      if (_pass != null && _pass != undefined) {
        ...
        _cp.id = myLocalId;
        this.groupeelecProduits.push(_cp);
      }
    }
    this.cancelProduit();
  }
  ...
  onSubmit() {
    ...
    this.groupeelecProduits = this.groupeelecProduits.filter((value, key) => {
      return !value.deleted;
    });
    
    let cp = this.groupeelecProduits.length == 0 ? [] : this.groupeelecProduits.map((value) => {
    ...
    this.local_data.groupeelecProduits = cp;
