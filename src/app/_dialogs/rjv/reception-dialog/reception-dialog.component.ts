...
@Component({
  ...
  styleUrls: ['./reception-dialog.component.scss']
})
export class ReceptionDialogComponent implements OnInit, AfterContentInit {
  ...
  ngOnInit(): void {
    ...
    this.formData = this.formBuilder.group({
      commandeProduit: new FormControl(this.local_data.commandeProduit),
      qte: new FormControl(this.local_data.qte, Validators.compose([
        Validators.required
      ])),
      oc: new FormControl(this.local_data.oc, Validators.compose([
        Validators.pattern(alphaNumAll), Validators.maxLength(15)
      ])),
    ...
    this.receptionProduits = this.local_data.receptionProduits || [];
  ...
  canMoveNext(): boolean {
    ...
    //return this.formData.valid && this.receptionProduits && this.receptionProduits.length > 0 && this.tempIndexProduit < 0;
    return this.receptionProduits && this.receptionProduits.length > 0 && this.tempIndexProduit < 0 && !this.formData.valid;
  }
  ...
  cancelProduit() {
    this.formData.controls['commandeProduit'].reset();
    this.formData.controls['oc'].reset();
    this.formData.controls['obs'].reset();
    this.formData.controls['qte'].reset();
    this.tempIndexProduit = -1;
  }

  editProduit(_data1, i) {
    delete _data1.deleted;
    this.formData.get('commandeProduit').setValue(_data1.commandeProduit);
    this.formData.get('oc').setValue(_data1.oc);
    this.formData.get('obs').setValue(_data1.obs);
    this.formData.get('qte').setValue(_data1.qte);
    this.tempIndexProduit = i;
  }

  deleteProduit(i) {
    //this.receptionProduits.splice(i, 1);
    this.receptionProduits[i].deleted = true;
  }

  addProduit(_data1) {

    let _cp = this.receptionProduits[this.tempIndexProduit] || {};

    const myLocalId = _cp.id ?? Guid.create().toString();

    let prixStruct = this.zoneProduits.filter((x)=>{
      return x.produitId == _data1.commandeProduit.stationProduit.produitId;
    })[0];
    ...
    _cp.commandeProduit = _data1.commandeProduit;
    _cp.commandeProduitId = _data1.commandeProduit.id;
    ...
    if (this.tempIndexProduit >= 0) {
      ...
      let _pass = this.receptionProduits.filter((x) => {
        return (x.id == _cp.id);
      })[0];
      if (_pass != null && _pass != undefined) {
        ...
        _cp.id = myLocalId;
        this.receptionProduits.push(_cp);
      }
    }
    this.cancelProduit();
  }
  ...
  onSubmit() {
    ...
    this.receptionProduits = this.receptionProduits.filter((value, key) => {
      return !value.deleted;
    });
    let cp = this.receptionProduits.length == 0 ? [] : this.receptionProduits.map((value) => {

      return {
        id: value.id,
        qte: value.qte,
        prixVente: value.prixVente,
        oc: (!isNullOrUndefined(value.oc) ? (value.oc + '').toLocaleUpperCase() : null),
        obs: (!isNullOrUndefined(value.obs) ? (value.obs + '').toLocaleUpperCase() : null),
        commandeProduit: value.commandeProduit,
        commandeProduitId: value.commandeProduit.id,
        stationProduit: value.commandeProduit.stationProduit,
        stationProduitId: value.commandeProduit.stationProduitId,
        rjvId: this.local_data.id,
      };
    })

    this.local_data.receptionProduits = cp;
    ...
    dialogRef.afterClosed().subscribe(async result => {
      ...
      if (!isNullOrUndefined(result) && !isNullOrUndefined(result.data)) {
        if (result.event == "Choix d'un produit") {
          ...
          let _pass = this.receptionProduits.filter((x) => {
            return typeof(x.id) === 'string' && (x.commandeProduitId == result.data[0].id);
          })[0];
          if(_pass == null || _pass == undefined){
            this.formData.get('commandeProduit').setValue(result.data[0]);
