...
@Component({
  selector: 'app-remisecuve-dialog',
  templateUrl: './remisecuve-dialog.component.html',
  styleUrls: ['./remisecuve-dialog.component.scss']
})
export class RemisecuveDialogComponent implements OnInit, AfterContentInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<RemisecuveDialogComponent>,
  ...
  ngOnInit(): void {
    ...
    this.remisecuveProduits = this.local_data.remisecuveProduits || [];
    if(this.local_data && 
  ...
  canMoveNext(): boolean {
    ...
    //return this.formData.valid && this.remisecuveProduits && this.remisecuveProduits.length > 0 && this.tempIndexProduit < 0;
    return this.remisecuveProduits && this.remisecuveProduits.length > 0 && this.tempIndexProduit < 0 && !this.formData.valid;
  }
  ...
  deleteProduit(i) {
    //this.remisecuveProduits.splice(i, 1);
    this.remisecuveProduits[i].deleted = true;
  }

  addProduit(_data1) {

    let _cp = this.remisecuveProduits[this.tempIndexProduit] || {};
    ...
    if (this.tempIndexProduit >= 0) {
      ...
      let _pass = this.remisecuveProduits.filter((x) => {
        return (x.id == _cp.id);
      })[0];
      if (_pass != null && _pass != undefined) {
        ...
        _cp.id = myLocalId;
        this.remisecuveProduits.push(_cp);
      }
    }
    this.cancelProduit();
  }
  ...
  onSubmit() {
    ...
    this.remisecuveProduits = this.remisecuveProduits.filter((value, key) => {
      return !value.deleted;
    });
    let cp = this.remisecuveProduits.length == 0 ? [] : this.remisecuveProduits.map((value) => {
    ...
    this.local_data.remisecuveProduits = cp;
