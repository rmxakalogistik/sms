...
@Component({
  ...
  styleUrls: ['./depense-dialog.component.scss']
})
export class DepenseDialogComponent implements OnInit, AfterContentInit {
  ...
  ngOnInit(): void {
    ...
    this.depenses = this.local_data.depenses || [];
  ...
  canMoveNext(): boolean {
    ...
    //return this.formData.valid && this.depenses && this.depenses.length > 0 && this.tempIndexProduit < 0;
    return this.depenses && this.depenses.length > 0 && this.tempIndexProduit < 0 && !this.formData.valid;
  }
  ...
  deleteProduit(i) {
    ...
    this.depenses[i].deleted = true;
  }

  addProduit(_data1) {

    let _cp = this.depenses[this.tempIndexProduit] || {};
    ...
    if (this.tempIndexProduit >= 0) {
      ...
      let _pass = this.depenses.filter((x) => {
        return (x.id == _cp.id);
      })[0];
  ...
  onSubmit() {
    ...
    this.depenses = this.depenses.filter((value, key) => {
      return !value.deleted;
    });
    
    let cp = this.depenses.length == 0 ? [] : this.depenses.map((value) => {
