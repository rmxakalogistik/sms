...
export class ZonesComponent implements OnInit {
  ...
  zoneSearchUpdate = new Subject<string>();
  env: EnvService;

  // ViewChild
  @ViewChild('zonetable', { static: true }) zonetable: MatTable<any>;
  @ViewChild('myPaginator', { static: false }) paginator: MatPaginator;
  @ViewChild('inputZoneSearch', { static: true }) inputZoneSearch: ElementRef;
  ...
  ngOnInit(): void {
    ...
    this.zoneDisplayedColumns = ['nom','produit', 'prix', 'action'];//'special',
    ...
    if (this.authService.roleEXist('zones_manage').add) {
  ...
  applyFilter(valllue = null) {
    ...
    // this.zonetable.dataSource.filter = filterValue.trim().toLowerCase();
    this.loadZone(this.filterValue?.trim().toUpperCase());
    if (this.paginator) {
        ...
        merge(this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.zoneService!.getAll(this.paginator.pageIndex, this.paginator.pageSize || 5, search);
            }),
            map(data => {
          ...
          this.zones = data;
          if (!isNullOrUndefined(this.zones)) {
            this.clearNewLogoAlbum();
            for (let i = 0; i < this.zones.length; i++) {
              let element = this.zones[i];
  ...
  openZoneDialog(action, obj) {
    obj.action = action;

    const dialogRef = this.popupService.open(ZoneDialogComponent, '800px', {
      data: obj,
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (!isNullOrUndefined(result)) {
        if (result.event == 'Création') {
          this.addZoneRowData(result.data);
        } else if (result.event == 'Modification') {
          this.updateZoneRowData(result.data);
        } else if (result.event == 'Suppression') {
          this.deleteZoneRowData(result.data);
        }
      }
    });
  }
  addZoneRowData(row_obj) {
    //// console.log(row_obj);
    // var d = new Date();
    const hjj = this.zones.filter((value, key) => {
      return value.id == row_obj.id;
    })[0];
    if (isNullOrUndefined(hjj)) {
      this.zones.splice(0, 0, row_obj);
    }
    this.zonetable.renderRows();

  }
  updateZoneRowData(row_obj) {
    this.zones = this.zones.filter((value, key) => {
  ...
  deleteZoneRowData(row_obj) {
    this.zones = this.zones.filter((value, key) => {
      return value.id != row_obj.id;
    });
  }
  openZoneBottomSheet(element) {
    ...
    if (this.authService.roleEXist('zones_manage')?.edit) {
      _actions.push({ text: 'Modification', icon: 'create' });
    }

    if (this.authService.roleEXist('zones_manage')?.delete) {
      ...
      this.bottomSheetRef.afterDismissed().subscribe(bottomSheetresult => {
        ...
        if (!isNullOrUndefined(result)) {
          if (result.text == "Ajouter un utilisateur") {
            this.openUserDialog('Création', { isActive: true, zone:element, zoneId:element.id }, element);
          }
          else {
            this.openZoneDialog(result.text, element);
          }
        }
      });
    }
  ...
  //    steps: [
  //      {
  //        intro: "Bienvenue dans la gestion des zones!"
  //      },
  //      //{
  ...
  doFabAction(action) {
    if (action == 'add') {
      this.openZoneDialog('Création', { isActive: true });
  ...
  setNewLogoAlbum(element) {
    const src = (element.docphoto > 0) ? this._env.API_URL + '/upload/zones/logos/' + element.docphoto + '.jpeg' : this.defaultImage;
    const caption = element.title;
    const thumb = (element.docphoto > 0) ? this._env.API_URL + '/upload/zones/logos/' + element.docphoto + '.jpeg' : this.defaultImage;
    const album = {
      src: src,
      caption: caption,
      thumb: thumb
