...
import { BanqueService } from '../../_services/banque.service';
...
export class BanquesComponent implements OnInit {
  ...
  banqueSearchUpdate = new Subject<string>();
  env: EnvService;

  // ViewChild
  @ViewChild('banquetable', { static: true }) banquetable: MatTable<any>;
  @ViewChild('myPaginator', { static: false }) paginator: MatPaginator;
  @ViewChild('inputBanqueSearch', { static: true }) inputBanqueSearch: ElementRef;
  ...
  ngOnInit(): void {
    ...
    this.banqueDisplayedColumns = ['nom','checquier', 'action'];//'special',
    ...
    if (this.authService.roleEXist('banques_manage').add) {
  ...
  applyFilter(valllue = null) {
    ...
    // this.banquetable.dataSource.filter = filterValue.trim().toLowerCase();
    this.loadBanque(this.filterValue?.trim().toUpperCase());
    if (this.paginator) {
        ...
        merge(this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.banqueService!.getAll(this.paginator.pageIndex, this.paginator.pageSize || 5, search);
            }),
            map(data => {
          ...
          this.banques = data;
          if (!isNullOrUndefined(this.banques)) {
            this.clearNewLogoAlbum();
            for (let i = 0; i < this.banques.length; i++) {
              let element = this.banques[i];
  ...
  openBanqueDialog(action, obj) {
    obj.action = action;

    const dialogRef = this.popupService.open(BanqueDialogComponent, '800px', {
      data: obj,
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (!isNullOrUndefined(result)) {
        if (result.event == 'Création') {
          this.addBanqueRowData(result.data);
        } else if (result.event == 'Modification') {
          this.updateBanqueRowData(result.data);
        } else if (result.event == 'Suppression') {
          this.deleteBanqueRowData(result.data);
        }
      }
    });
  }
  addBanqueRowData(row_obj) {
    //// console.log(row_obj);
    // var d = new Date();
    const hjj = this.banques.filter((value, key) => {
      return value.id == row_obj.id;
    })[0];
    if (isNullOrUndefined(hjj)) {
      this.banques.splice(0, 0, row_obj);
    }
    this.banquetable.renderRows();

  }
  updateBanqueRowData(row_obj) {
    this.banques = this.banques.filter((value, key) => {
  ...
  deleteBanqueRowData(row_obj) {
    this.banques = this.banques.filter((value, key) => {
      return value.id != row_obj.id;
    });
  }
  openBanqueBottomSheet(element) {
    ...
    if (this.authService.roleEXist('banques_manage')?.edit) {
      _actions.push({ text: 'Modification', icon: 'create' });
    }

    if (this.authService.roleEXist('banques_manage')?.delete) {
      ...
      this.bottomSheetRef.afterDismissed().subscribe(bottomSheetresult => {
        ...
        if (!isNullOrUndefined(result)) {
          if (result.text == "Ajouter un utilisateur") {
            this.openUserDialog('Création', { isActive: true, banque:element, banqueId:element.id }, element);
          }
          else {
            this.openBanqueDialog(result.text, element);
          }
        }
      });
    }
  ...
  //    steps: [
  //      {
  //        intro: "Bienvenue dans la gestion des banques!"
  //      },
  //      //{
  ...
  doFabAction(action) {
    if (action == 'add') {
      this.openBanqueDialog('Création', { isActive: true });
  ...
  setNewLogoAlbum(element) {
    const src = (element.docphoto > 0) ? this._env.API_URL + '/upload/banques/logos/' + element.docphoto + '.jpeg' : this.defaultImage;
    const caption = element.title;
    const thumb = (element.docphoto > 0) ? this._env.API_URL + '/upload/banques/logos/' + element.docphoto + '.jpeg' : this.defaultImage;
    const album = {
      src: src,
      caption: caption,
      thumb: thumb
