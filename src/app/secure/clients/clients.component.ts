...
export class ClientsComponent implements OnInit {
  ...
  clientSearchUpdate = new Subject<string>();
  env: EnvService;

  // ViewChild
  @ViewChild('clienttable', { static: true }) clienttable: MatTable<any>;
  @ViewChild('myPaginator', { static: false }) paginator: MatPaginator;
  @ViewChild('inputClientSearch', { static: true }) inputClientSearch: ElementRef;
  ...
  ngOnInit(): void {
    ...
    this.clientDisplayedColumns = ['nom','produit','zone', 'prix', 'action'];//'special',
    ...
    this.signalrService.notifConnexionChanged.subscribe((_signalR_connection: any) => {
      if (!isNullOrUndefined(_signalR_connection) && !isNullOrUndefined(_signalR_connection.id)) {
        ...
        const ClientUpdatedlistener = this.signalR_connection.listenFor('newClientUpdated');
        ClientUpdatedlistener.subscribe(async (data: string) => {
    ...
    if (this.authService.roleEXist('clients_manage').add) {
  ...
  applyFilter(valllue = null) {
    ...
    // this.clienttable.dataSource.filter = filterValue.trim().toLowerCase();
    this.loadClient(this.filterValue?.trim().toUpperCase());
    if (this.paginator) {
        ...
        merge(this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.clientService!.getAll(this.paginator.pageIndex, this.paginator.pageSize || 5, search);
            }),
            map(data => {
  ...
  openClientDialog(action, obj) {
    obj.action = action;

    const dialogRef = this.popupService.open(ClientDialogComponent, '800px', {
      data: obj,
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (!isNullOrUndefined(result)) {
        if (result.event == 'Création') {
          this.addClientRowData(result.data);
        } else if (result.event == 'Modification') {
          this.updateClientRowData(result.data);
        } else if (result.event == 'Suppression') {
          this.deleteClientRowData(result.data);
        }
      }
    });
  }
  addClientRowData(row_obj) {
    //// console.log(row_obj);
    // var d = new Date();
    const hjj = this.clients.filter((value, key) => {
      return value.id == row_obj.id;
    })[0];
    if (isNullOrUndefined(hjj)) {
      this.clients.splice(0, 0, row_obj);
    }
    this.clienttable.renderRows();

  }
  updateClientRowData(row_obj) {
    this.clients = this.clients.filter((value, key) => {
  ...
  deleteClientRowData(row_obj) {
    this.clients = this.clients.filter((value, key) => {
      return value.id != row_obj.id;
    });
  }
  openClientBottomSheet(element) {
    ...
    if (this.authService.roleEXist('clients_manage')?.edit) {
      _actions.push({ text: 'Modification', icon: 'create' });
    }

    if (this.authService.roleEXist('clients_manage')?.delete) {
      ...
      this.bottomSheetRef.afterDismissed().subscribe(bottomSheetresult => {
        ...
        if (!isNullOrUndefined(result)) {
          if (result.text == "Ajouter un utilisateur") {
            this.openUserDialog('Création', { isActive: true, client:element, clientId:element.id }, element);
          }
          else {
            this.openClientDialog(result.text, element);
          }
        }
      });
    }
  ...
  //    steps: [
  //      {
  //        intro: "Bienvenue dans la gestion des clients!"
  //      },
  //      //{
  ...
  doFabAction(action) {
    if (action == 'add') {
      this.openClientDialog('Création', { isActive: true });
  ...
  setNewLogoAlbum(element) {
    const src = (element.docphoto > 0) ? this._env.API_URL + '/upload/clients/logos/' + element.docphoto + '.jpeg' : this.defaultImage;
    const caption = element.title;
    const thumb = (element.docphoto > 0) ? this._env.API_URL + '/upload/clients/logos/' + element.docphoto + '.jpeg' : this.defaultImage;
    const album = {
      src: src,
      caption: caption,
      thumb: thumb
