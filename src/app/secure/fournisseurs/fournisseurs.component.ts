...
export class FournisseursComponent implements OnInit {
  ...
  fournisseurDisplayedColumns: string[] = [];
  ...
  fournisseurSearchUpdate = new Subject<string>();
  env: EnvService;

  // ViewChild
  @ViewChild('fournisseurtable', { static: true }) fournisseurtable: MatTable<any>;
  @ViewChild('myPaginator', { static: false }) paginator: MatPaginator;
  @ViewChild('inputFournisseurSearch', { static: true }) inputFournisseurSearch: ElementRef;
  ...
  ngOnInit(): void {
    ...
    this.fournisseurDisplayedColumns = ['nom','produit','zone', 'prix', 'action'];//'special',
    ...
    this.signalrService.notifConnexionChanged.subscribe((_signalR_connection: any) => {
      if (!isNullOrUndefined(_signalR_connection) && !isNullOrUndefined(_signalR_connection.id)) {
        ...
        const FournisseurUpdatedlistener = this.signalR_connection.listenFor('newFournisseurUpdated');
        FournisseurUpdatedlistener.subscribe(async (data: string) => {
    ...
    // Debounce search.
    this.fournisseurSearchUpdate.pipe(
      debounceTime(100),
    ...
    if (this.authService.roleEXist('fournisseurs_manage').add) {
  ...
  applyFilter(valllue = null) {
    ...
    // this.fournisseurtable.dataSource.filter = filterValue.trim().toLowerCase();
    this.loadFournisseur(this.filterValue?.trim().toUpperCase());
    if (this.paginator) {
        ...
        merge(this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.fournisseurService!.getAll(this.paginator.pageIndex, this.paginator.pageSize || 5, search);
            }),
            map(data => {
          ...
          if (!isNullOrUndefined(this.fournisseurs)) {
            this.clearNewLogoAlbum();
            for (let i = 0; i < this.fournisseurs.length; i++) {
              let element = this.fournisseurs[i];
  ...
  openFournisseurDialog(action, obj) {
    obj.action = action;

    const dialogRef = this.popupService.open(FournisseurDialogComponent, '800px', {
      data: obj,
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (!isNullOrUndefined(result)) {
        if (result.event == 'Création') {
          this.addFournisseurRowData(result.data);
        } else if (result.event == 'Modification') {
          this.updateFournisseurRowData(result.data);
        } else if (result.event == 'Suppression') {
          this.deleteFournisseurRowData(result.data);
        }
      }
    });
  }
  addFournisseurRowData(row_obj) {
    //// console.log(row_obj);
    // var d = new Date();
    const hjj = this.fournisseurs.filter((value, key) => {
      return value.id == row_obj.id;
    })[0];
    if (isNullOrUndefined(hjj)) {
      this.fournisseurs.splice(0, 0, row_obj);
    }
    this.fournisseurtable.renderRows();

  }
  updateFournisseurRowData(row_obj) {
    this.fournisseurs = this.fournisseurs.filter((value, key) => {
  ...
  deleteFournisseurRowData(row_obj) {
    this.fournisseurs = this.fournisseurs.filter((value, key) => {
      return value.id != row_obj.id;
    });
  }
  openFournisseurBottomSheet(element) {
    ...
    if (this.authService.roleEXist('fournisseurs_manage')?.edit) {
      _actions.push({ text: 'Modification', icon: 'create' });
    }

    if (this.authService.roleEXist('fournisseurs_manage')?.delete) {
      ...
      this.bottomSheetRef.afterDismissed().subscribe(bottomSheetresult => {
        ...
        if (!isNullOrUndefined(result)) {
          if (result.text == "Ajouter un utilisateur") {
            this.openUserDialog('Création', { isActive: true, fournisseur:element, fournisseurId:element.id }, element);
          }
          else {
            this.openFournisseurDialog(result.text, element);
          }
        }
      });
    }
  ...
  //    steps: [
  //      {
  //        intro: "Bienvenue dans la gestion des fournisseurs!"
  //      },
  //      //{
  ...
  doFabAction(action) {
    if (action == 'add') {
      this.openFournisseurDialog('Création', { isActive: true });
  ...
  setNewLogoAlbum(element) {
    const src = (element.docphoto > 0) ? this._env.API_URL + '/upload/fournisseurs/logos/' + element.docphoto + '.jpeg' : this.defaultImage;
    const caption = element.title;
    const thumb = (element.docphoto > 0) ? this._env.API_URL + '/upload/fournisseurs/logos/' + element.docphoto + '.jpeg' : this.defaultImage;
    const album = {
      src: src,
      caption: caption,
      thumb: thumb
