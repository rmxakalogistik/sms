...
@Component({
  selector: 'app-commandePrintable',
  templateUrl: './commandePrintable.component.html',
  styleUrls: ['./commandePrintable.component.scss']
})
export class CommandePrintableComponent implements OnInit {
  ...
  sumDataList(list = []) {
    if (list.length > 0) {
      const filtered_enll = list.filter((value) => {
        return true;
      });
      return filtered_enll.reduce((a, b) => a + ((b.qte*b.prix) || 0), 0);
    }
  }
